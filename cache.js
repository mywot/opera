...
$.extend(wot, { cache: {
	...
	setflags: function(name, flags)
	{
		try {
			...
			wot.log("cache.setflags: failed with " + e + "\n", true);
		}
	},
	...
	set: function(name, status, value)
	{
		try {
			...
			wot.log("cache.set: failed with " + e + "\n", true);
		}
	...
	get: function(name)
	{
		try {
			...
			wot.log("cache.get: failed with " + e + "\n", true);
		}
	...
	clear: function(name)
	{
		try {
			...
			wot.log("cache.clear: failed with " + e + "\n", true);
		}
	...
	cacheratingstate: function(name, state)
	{
		try {
			...
			wot.log("cache.cacheratingstate: failed with " + e + "\n", true);
		}
	...
	cacheresponse: function(hosts, data, status)
	{
		...
		try {
			...
			wot.log("cache.cacheresponse: failed with " + e + "\n", true);
		}
